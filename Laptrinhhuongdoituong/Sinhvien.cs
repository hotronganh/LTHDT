...
namespace Laptrinhhuongdoituong
{
    class Sinhvien
    {
        private string sbd, kv, doituong, uutien, dantoc, ten, ngaysinh;
       ...
       private double diemcacmon( string str)
        ...
        private double diemdoituong(string s)
        {
            switch (s)
            {
                case "\"NDT1\"": diemdt = 2; break;
                case "\"NDT2\"": diemdt = 2; break;
                case "\"NDT3\"": diemdt = 2; break;
                case "\"NDT4\"": diemdt = 2; break;
            ...
            for (int i = 0; i < 4; i++)
                for (int j = 1; j < 2; j++)
                {
                    ...
                    else
                    {
                        ...
                        if (s[3] == "1")
                        {
                            ketqua[i] = (diemcacmon(s[0]) * 2 + diemcacmon(s[1]) + diemcacmon(s[2]) + diemdoituong(doituong) + diemkhuvuc(kv)) / 4 + diemuutien(uutien);
                        }
                        else
                            ketqua[i] = (diemcacmon(s[0]) + diemcacmon(s[1]) + diemcacmon(s[2]) + diemdoituong(doituong) + diemkhuvuc(kv)) / 3 + diemuutien(uutien);
                    }
        ...
        {
            SQLiteConnection conn = new SQLiteConnection(@"Data Source = D:\SQLite-thuvienwinform\02.db");
            conn.Open();
